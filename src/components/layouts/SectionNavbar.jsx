...
const SectionNavbar = ({ sectionName, dropdown = [], pathname = "", productsMenuData = null, isLoadingProducts = false }) => {
  if (!Array.isArray(dropdown) || dropdown.length === 0) return null;

  switch (sectionName) {
    case "Products":
      // Get Strapi data or use fallback
      const strapiData = productsMenuData[0];
  console.log(strapiData,"strapiData")

      const productsData = dropdown;
      
      // Create centralized data object with Strapi data and fallbacks
      const productsSectionData = {
        sections: {
          readyToUse: {
            title: strapiData?.readyToUse?.title || "OFF-THE-SHELF",
            product: {
              ...productsData[0],
              name: strapiData?.readyToUse?.name || productsData[0]?.name,
              descrip: strapiData?.readyToUse?.descrip || productsData[0]?.descrip,
              path: productsData[0]?.path,
              Arraypath: productsData[0]?.path
            }
          },
          buildYourOwn: {
            title: strapiData?.buildYourOwn?.title || "BUILD YOUR OWN PROGRAM",
            product: {
              ...productsData[1],
              name: strapiData?.buildYourOwn?.name || productsData[1]?.name,
              descrip: strapiData?.buildYourOwn?.descrip || productsData[1]?.descrip,
              path: productsData[1]?.path,
              Arraypath: productsData[1]?.path
            }
          }
        },
        ...
        capabilities: strapiData?.capabilities || [
          ...
          <div className="flex-1 bg-gradient-to-br from-blue-50 to-indigo-50 text-left p-6 rounded-l-xl border-r border-gray-100 max-w-[300px]">
            <div className="mb-6">
              <span className="text-xs font-semibold text-blue-600 bg-blue-100 px-3 py-1 rounded-full">
                {productsSectionData.sections.readyToUse.title}
              </span>
            </div>

            <div className="space-y-3">
              <Link
                key={productsSectionData.sections.readyToUse.product.path}
                href={productsSectionData.sections.readyToUse.product.path}
                className={`group flex items-start gap-3 p-4 rounded-lg transition-all duration-30 ${pathname.includes(productsSectionData.sections.readyToUse.product.Arraypath)
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-medium text-gray-900 mb-1  transition-colors text-sm">
                    {productsSectionData.sections.readyToUse.product.name}
                  </h3>
                  <p className="text-xs text-gray-600 leading-tight">
                    {productsSectionData.sections.readyToUse.product.descrip}
                  </p>
                </div>
              </Link>
            </div>
            ...
            <div className="my-6">
              <span className="text-xs font-semibold text-blue-600 bg-blue-100 px-3 py-1 rounded-full">
                {productsSectionData.sections.buildYourOwn.title}
              </span>
            </div>

            <div className="space-y-3">
              <Link
                key={productsSectionData.sections.buildYourOwn.product.path}
                href={productsSectionData.sections.buildYourOwn.product.path}
                className={`group flex items-start gap-3 p-4 rounded-lg transition-all duration-30 ${pathname.includes(productsSectionData.sections.buildYourOwn.product.Arraypath)
                    ? " shadow-lg border border-blue-200 transform scale-[1.02] -translate-y-1"
                    : "hover: hover:shadow-2xl hover:scale-[1.02] hover:-translate-y-1 border border-transparent"
                  }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-medium text-gray-900 mb-1  transition-colors text-sm">
                    {productsSectionData.sections.buildYourOwn.product.name}
                  </h3>
                  <p className="text-xs text-gray-600 leading-tight">
                    {productsSectionData.sections.buildYourOwn.product.descrip}
                  </p>
                </div>
              </Link>
            </div>
          ...
          <div className="flex flex-1 bg-white rounded-r-xl">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-1">
                {productsSectionData.features.map(({ name, icon }) => (
                  <div
                    key={name}
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-1">
                {productsSectionData.capabilities.map(({ name, icon }) => (
                  <div
                    key={name}
          ...
          <div className="grid grid-cols-1 gap-2 ">
            {dropdown.map(({ name, path, icon, descrip }) => (
              <Link
                key={path}
                href={path}
                className={` rounded-lg transition-colors ${pathname.includes(path)
